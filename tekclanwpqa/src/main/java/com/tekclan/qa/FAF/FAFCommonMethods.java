...
public class FAFCommonMethods extends CommandUtils {
	static WebDriverWait wait;
	public static String generatedString = RandomStringUtils.randomAlphabetic(10);
	
	public static void faf_Login(WebDriver driver) throws InterruptedException
	{
		wait = new WebDriverWait(driver,30);
		//		Get FAF URL
		driver.get(Directory.faf_qa_url);
		driver.manage().window().maximize();
		//		Username field
		wait.until(ExpectedConditions.elementToBeClickable(By.id(OR.USERNAME_FIELD)));
		driver.findElement(By.id(OR.USERNAME_FIELD)).sendKeys(Directory.faf_qa_username);

		//		Password field
		wait.until(ExpectedConditions.elementToBeClickable(By.id(OR.PASSWORD_FIELD)));
		driver.findElement(By.id(OR.PASSWORD_FIELD)).sendKeys(Directory.faf_qa_password);

		//		Click submit button
		wait.until(ExpectedConditions.elementToBeClickable(By.id(OR.SUBMIT_BUTTOM)));
		driver.findElement(By.id(OR.SUBMIT_BUTTOM)).click();

		System.out.println("Signed in with -  " + Directory.faf_qa_username);
		Thread.sleep(2000);

		//		Navigate to Executive portal URL
		driver.navigate().to(Directory.faf_qa_navigate_url);
		Thread.sleep(1000);
	}	
